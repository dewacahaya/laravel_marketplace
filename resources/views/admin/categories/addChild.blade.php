...
@section('admin_content')
    <div class="p-6 bg-white dark:bg-gray-900 rounded-lg shadow">
        <h2 class="text-xl font-semibold mb-4">Add {{ $parent->name }} Child Category</h2>

        <form action="{{ route('admin.categories.storeChild') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label class="block mb-1">Child Category Name</label>
            ...
            @if (isset($parent))
                ...
                <p class="text-sm text-gray-400">Adding child under: <strong>{{ $parent->name }}</strong></p>
            @endif
