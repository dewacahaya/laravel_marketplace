...
@section('vendor_content')
    <div class="container mx-auto px-4 py-8 md:py-12">
        <div class="flex justify-between items-center mb-6 border-b pb-2">
            <h2 class="text-3xl font-extrabold text-gray-900">Detail Pesanan #{{ $order->id }}</h2>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-lg font-medium text-gray-900">{{ $order->user->name ?? 'N/A' }}</p>
                <p class="text-gray-700">Email: {{ $order->user->email ?? 'N/A' }}</p>
            </div>

            <div>
                <h3 class="text-xl font-semibold text-gray-800 mb-3 border-b pb-2">Alamat Pengiriman</h3>
                <p class="font-medium text-gray-900">{{ $order->address ?? 'Alamat tidak tersedia' }}</p>
                <p class="text-gray-700">Telp: {{ $order->phone ?? 'N/A' }}</p>
                @if ($order->note)
                    <p class="mt-3 text-sm italic text-gray-500">Catatan: {{ $order->note }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white shadow-xl rounded-lg overflow-hidden mt-8">
            <h3 class="text-xl font-semibold text-gray-800 p-4 border-b bg-gray-50">
                Produk Anda dalam Pesanan Ini ({{ $order->items->count() }} Item)
            </h3>

            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                        <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/2">Produk
                        </th>
                        <th class="p-4 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Qty</th>
                        <th class="p-4 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Harga Satuan
                        </th>
                        <th class="p-4 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Subtotal Anda
                        </th>
                        <th class="p-4 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Aksi Item
                        </th>
                    </tr>
                </thead>
                ...
                <tfoot>
                    <tr>
                        <td colspan="3" class="p-4 text-right text-xl font-bold text-gray-800">Total Pendapatan Anda:
                        </td>
